...
**Agregado en migración 004** (`database/migrations/004_pnid_import.sql`, importación P&ID/Plant 3D — ver `CLAUDE.md`): `tag_anterior NVARCHAR(50) NULL` (reemplaza el concepto "TAG_WSP" diferido, ver `MATRIZ_COBERTURA_DATOS_SIEI.md` sección 2 y 11; nunca autogenerado al detectar `TAG_MODIFICADO`) · `tecnologia NVARCHAR(100) NULL` · `funcionamiento NVARCHAR(50) NULL` texto libre, sin catálogo · `cuerpo_instrumento NVARCHAR(50) NULL` texto libre, sin catálogo · `conexion_proceso NVARCHAR(100) NULL` · `plano_pnid NVARCHAR(30) NULL` · `linea_pnid NVARCHAR(100) NULL` · `tipo_senal_pnid NVARCHAR(50) NULL` (anotación de P&ID, sin FK a catálogo de señales, no crea `nucleo.senal`) · `equipo_asociado_id BIGINT NULL`, `FK (equipo_asociado_id, proyecto_id) → nucleo.equipo (id, proyecto_id)`, `IX` filtrado `WHERE equipo_asociado_id IS NOT NULL` — relación distinta de `senal.equipo_id`, no reabre la decisión "EQUIPO sin relación con INSTRUMENTO" (ver `MATRIZ_COBERTURA_DATOS_SIEI.md` sección 11) · `equipo_asociado_tag NVARCHAR(50) NULL` (texto literal del P&ID, siempre conservado aunque no resuelva a un `equipo` existente — nunca se crea un `equipo` ficticio). A partir de esta migración, `pnpid`/`fuente_pnpid` dejan de ser editables por `POST`/`PATCH` de `/instruments` — solo el importador (o acceso directo a la base) los escribe.

**Agregado en migración 005** (`database/migrations/005_instrumento_asociado.sql` — columna "Instrumento Asociado" agregada por el usuario al reporte P&ID después de 004): `instrumento_asociado_id BIGINT NULL`, `FK (instrumento_asociado_id, proyecto_id) → nucleo.instrumento (id, proyecto_id)` (auto-referencia, mismo aislamiento multi-proyecto que el resto del modelo), `IX` filtrado `WHERE instrumento_asociado_id IS NOT NULL`, `CK_instrumento_asociado_no_self CHECK (instrumento_asociado_id IS NULL OR instrumento_asociado_id <> id)` · `instrumento_asociado_tag NVARCHAR(50) NULL` (texto literal del P&ID, mismo patrón que `equipo_asociado_tag`: siempre conservado, nunca crea un instrumento ficticio para resolver la relación). Vínculo INSTRUMENTO↔INSTRUMENTO — "el instrumento contiene al otro instrumento asociado" —, modelado y sincronizado exactamente igual que `equipo_asociado_id`/`_tag`: participa en `DIFFABLE_FIELDS` (un cambio en `instrumento_asociado_tag` produce `DATOS_MODIFICADOS`, igual que `equipo_asociado_tag`).

#### 8.3.1 Schema `integracion` — importación P&ID (migración 004)

Persistente para siempre (no efímero — decisión revisada, ver `MODELO_CONCEPTUAL_SIEI.md` sección 3). Tres tablas:

- **`integracion.importacion_pnid`** — cabecera de un batch: `proyecto_id FK`, `nombre_archivo`, `hash_archivo CHAR(64)` (informativo, nunca bloquea reimportar), `fuente NVARCHAR(50) DEFAULT 'PLANT3D'`, `estado NVARCHAR(20) CHECK IN ('PREVISUALIZADO','APLICADO','DESCARTADO','ERROR')`, contadores por resultado, `advertencias NVARCHAR(MAX)` JSON (`ISJSON` CHECK) con `missingKnownColumns`/`unknownColumns`, `created_by`/`applied_by FK → seguridad.usuario`, `UQ(id, proyecto_id)`.
- **`integracion.importacion_pnid_fila`** — snapshot puro, exclusivamente las filas físicas del archivo (una por cada fila del Excel, sin excepción, `Listado=False` incluido): `numero_fila`, `pnpid`/`tag_instrumento` normalizados, `listado BIT`, `datos_fuente NVARCHAR(MAX)` JSON con **todas** las columnas del archivo bajo su encabezado original (`ISJSON` CHECK), `UQ(id, proyecto_id)`, `UQ(importacion_id, numero_fila)`, `FK (importacion_id, proyecto_id) → importacion_pnid (id, proyecto_id)`.
- **`integracion.importacion_pnid_resultado`** — conclusiones de la comparación, separadas del snapshot: `fila_id BIGINT NULL` (FK compuesta a `importacion_pnid_fila`; `NULL` para `NO_EXISTE_EN_PNID` sin fila fuente — instrumento administrado por `PLANT3D` que desapareció del reporte), `instrumento_id BIGINT NULL` (FK compuesta a `nucleo.instrumento`), `resultado_id BIGINT NOT NULL FK → cat.cat_estado_pnid`, `diferencias NVARCHAR(MAX) NULL` JSON (`ISJSON` CHECK; siempre objeto/array, nunca string suelto — `ISJSON` de SQL Server exige nivel superior objeto o array), `instrumento_updated_at_preview DATETIME2 NULL` (snapshot de concurrencia para `APPLY`), `CK (fila_id IS NOT NULL OR instrumento_id IS NOT NULL)`, `UX(importacion_id, fila_id) WHERE fila_id IS NOT NULL`, `UX(importacion_id, instrumento_id) WHERE instrumento_id IS NOT NULL`.

`cat.cat_estado_pnid` se extendió con `DATOS_MODIFICADOS` y `REQUIERE_REVISION` (los otros 7 códigos ya existentes se reutilizan tal cual para `resultado_id` — ver `MATRIZ_COBERTURA_DATOS_SIEI.md` sección 11 para el mapeo completo).
...
**Agregado en migración 007** (`database/migrations/007_equipos_instrumentacion.sql`, catálogo curado de Equipos — ver `CLAUDE.md` sección "Equipos"): `plano_pnid NVARCHAR(50) NULL` (referencia de documento P&ID, sin relación con el importador P&ID) · `tipo_equipo_id BIGINT NULL FK → cat.cat_tipo_equipo` (ver 2.x). `nucleo.equipo` sigue siendo un catálogo poblado a mano, nunca automáticamente desde un reporte.
...
#### 8.20 Entregables (migración 006) — primer módulo real: LDI

Filosofía de 3 capas: MASTER (`nucleo.instrumento`, sin cambios) → ENTREGABLE (documento, numeración propia) → REVISION_ENTREGABLE (snapshot histórico congelado — nunca relee el master ni el reporte P&ID). Detalle completo de las decisiones de negocio en `CLAUDE.md` sección "Entregables / LDI".

- **`cat.cat_tipo_entregable`** *(global, sin `proyecto_id`)*: `id`, `codigo` (`UQ`, ej. `'LDI'`), `descripcion` (ej. `'LISTADO DE INSTRUMENTOS'`), `disciplina` (conceptual, ej. `'Instrumentación'` — **no** la letra documental de numeración, esa vive en `entregable.componente_disciplina` porque depende del proyecto/cliente, no del tipo). Seed: 1 fila (`LDI`).
- **`cat.cat_orden_tipo_instrumento`** *(global)*: `id`, `prefijo` (`UQ`, ej. `'LIT'`), `orden` (`INT`). Preset inicial de Instrumentación (20 filas, LIT=10 … ZT=42) — evolucionable a `(proyecto_id NULL, prefijo)` si algún proyecto futuro necesita su propio preset, sin rediseño.
- **`nucleo.proyecto_documentacion`** *(1:1 con `proyecto_id`, `UQ`)*: metadatos de carátula heredables — `codigo_proyecto_cumbra`, `codigo_proyecto_cliente`, `titulo_caratula`, `etapa_codigo`/`etapa_nombre`, `afe`, `vp`, `jefe_disciplina`, `lider_proyecto`, `gerente_ingenieria_construccion`, `iniciales_por_default`/`_revisado_default`/`_aprobado_default`. Deliberadamente separada de `nucleo.proyecto` (identidad estructural) — `proyecto.codigo_proyecto` **no** se reutiliza para "Proyecto CUMBRA".
- **`nucleo.plantilla_entregable`**: `id`, `proyecto_id`, `tipo_entregable_id`, `nombre_archivo`, `archivo_blob VARBINARY(MAX)`, `archivo_hash CHAR(64)`, `tamanio_bytes`, `activo`. `UX (proyecto_id, tipo_entregable_id) WHERE activo=1` — una sola vigente a la vez; reemplazar es INSERT nuevo + desactivar la anterior, nunca UPDATE del blob (`TR_plantilla_entregable_blob_inmutable` lo rechaza incondicionalmente, la haya usado ya una revisión o no).
- **`nucleo.configuracion_orden`**: `id`, `proyecto_id`, `tipo_entregable_id` (nullable), `nombre`, `criterios_json` (`CHECK ISJSON`, `[{campo, direccion}]`), `es_default`, `activo`. `UX (proyecto_id, tipo_entregable_id) WHERE es_default=1 AND activo=1`. Reutilizable entre revisiones — cada revisión congela su propia copia en `criterios_aplicados_json`.
- **`nucleo.entregable`**: `id` (`UQ (id, proyecto_id)`), `proyecto_id`, `tipo_entregable_id`, `numero_documento` (`UQ (proyecto_id, numero_documento)`), y los componentes del número **congelados como texto** en el momento de creación — `componente_etapa`, `componente_proyecto`, `componente_cliente`, `componente_tipo`, `componente_area`, `componente_disciplina`, `componente_correlativo` (`UQ` compuesto de los 5 últimos + proyecto, protege reutilización accidental de la combinación), `titulo`, `activo`. No lleva `plantilla_id` ni `configuracion_orden_id` — esos conceptos son de proyecto+tipo, se resuelven y congelan por revisión, no por entregable.
- **`nucleo.revision_entregable`**: `id` (`UQ (id, proyecto_id)`), `entregable_id` (FK compuesta), `codigo_revision`, `fecha`, `descripcion`, `iniciales_por`/`_revisado`/`_aprobado`, `estado` (`CHECK IN ('BORRADOR','EMITIDA','DESCARTADA')`), `configuracion_orden_id`/`criterios_aplicados_json`/`metadatos_snapshot_json`/`plantilla_id`/`archivo_id` (FKs compuestas a sus respectivas tablas, congelados al generar el preview), `emitida_by`/`emitida_at`, `descartada_by`/`descartada_at`. `UX (entregable_id, codigo_revision) WHERE estado='EMITIDA'` (código no se repite una vez emitido) y `UX (entregable_id) WHERE estado='BORRADOR'` (un solo borrador abierto a la vez). `CK_revision_entregable_emitida_completa` exige los 6 campos de "congelado" no-NULL antes de permitir `estado='EMITIDA'`. `TR_revision_entregable_estado_final_inmutable` (`AFTER UPDATE`) rechaza cualquier `UPDATE` si la fila ya estaba `EMITIDA`/`DESCARTADA` — ninguna transición sale de un estado final.
- **`nucleo.revision_entregable_fila`**: `id`, `revision_id` (FK compuesta), `instrumento_id` (FK compuesta, `NOT NULL` — a diferencia de `importacion_pnid_resultado`, toda fila del LDI viene de un instrumento real), `item`, `datos_snapshot` (`CHECK ISJSON` — las columnas del entregable ya resueltas, genérico para no atar el esquema a las columnas específicas de un entregable puntual (19 en el caso del LDI hoy, ver CLAUDE.md)). `UQ (revision_id, item)`, `UQ (revision_id, instrumento_id)`. `TR_revision_entregable_fila_estado_final_inmutable` (`AFTER INSERT, UPDATE, DELETE`) cierra el atajo de tocar el snapshot sin pasar por el padre.
- **`nucleo.revision_entregable_archivo`**: `id`, `revision_id` (`UQ`, 1:1, FK compuesta), `nombre_archivo`, `mime_type`, `archivo_blob VARBINARY(MAX)`, `archivo_hash CHAR(64)`, `tamanio_bytes`. Tabla separada de `revision_entregable` (el blob no debe pesar cada lectura de metadatos). `TR_revision_entregable_archivo_inmutable` (`AFTER UPDATE`) rechaza cualquier `UPDATE`, sin condición — un archivo emitido nunca se reemplaza, se corrige con una revisión nueva.

**Nota técnica SQL Server**: `nucleo.revision_entregable` tiene un trigger `AFTER UPDATE` habilitado, así que ningún `UPDATE` sobre esa tabla puede usar `OUTPUT` sin `INTO` (error 334) — el backend hace un `SELECT` aparte después de cada `UPDATE` en vez de `OUTPUT INSERTED.*` directo al cliente (el `INSERT` de creación sí puede, el trigger es solo de `UPDATE`).
