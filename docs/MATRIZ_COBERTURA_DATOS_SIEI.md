...
|---|---|---|---|
| `TAG_WSP` **(DIFERIDO — no se agrega columna, ✅ decisión explícita)** | IMPORT_PNID, familia SENALES, MASTER_INSTRUMENTOS, COM, Tabla_Lazos (`TAG_INSTRUMENTO_WSP`) | Tag del instrumento en la ingeniería previa/WSP | WSP es solo **una** empresa/ingeniería previa concreta del proyecto 620 analizado — no un concepto universal. El concepto general de negocio es más amplio: **ORIGEN / INGENIERÍA PREVIA / TRAZABILIDAD** (de dónde viene un dato de ingeniería antes de entrar a SIEI, quién lo generó, con qué tag lo identificaba esa fuente). Permanece **DIFERIDO** al futuro módulo de Ingeniería Previa — no se agrega ninguna columna `tag_wsp` al núcleo. Esta fila se conserva aquí (no se borra) precisamente para que la evidencia no se pierda cuando se diseñe ese módulo. |
...
2. Atributos de instrumento sin columna ni catálogo: `posicion_normal`, `posicion_falla`, `alimentacion`, `protocolo`, `observacion`, y los candidatos a catálogo `funcionamiento`/`cuerpo_instrumento` — clasificados en la sección 2.1b, ninguno implementado todavía.
