...
import { deactivateRio, getRio } from '../api/rios';
...
import type { Channel, ModuleType, PhysicalModule, Rack, Rio, Slot } from '../api/types';
...
export function RioDetailPage() {
  const { projectId, rioId } = useParams<{ projectId: string; rioId: string }>();
  ...
  const fetchRio = useCallback(() => {
    if (!projectId || !rioId) return Promise.resolve<Rio | null>(null);
    return getRio(projectId, rioId, devUser.email).then((r) => r.rio);
  }, [projectId, rioId, devUser.email]);

  const { data: rio, loading: rioLoading, error: rioError, refresh: refreshRio } = useAsyncData<
    Rio | null
  >(fetchRio);
  ...
  async function handleAddRack(event: FormEvent) {
    ...
    try {
      await createRack(projectId!, { rioId: rioId!, numeroRack: Number(numeroRack) }, devUser.email);
      setNumeroRack('');
      refreshTree();
    } catch (err) {
  ...
  async function handleDeactivateRio() {
    if (!rio) return;
    if (!window.confirm(`¿Desactivar el RIO "${rio.tagRio}"?`)) return;
    setDeactivating(true);
    try {
      await deactivateRio(projectId!, rioId!, devUser.email);
      navigate(`/projects/${projectId}/rios`);
    } catch (err) {
      setAddRackError(err instanceof Error ? err : new Error('Error desconocido.'));
  ...
  const racksForRio = (tree?.racks ?? [])
    .filter((r) => r.rioId === rioId)
  ...
  const error = addRackError ?? rioError ?? treeError;
  const loading = rioLoading || treeLoading;

  return (
    <section>
      <div className="page-header">
        <h1>{rio ? rio.tagRio : 'RIO'}</h1>
        {rio && (
          <button
            type="button"
            ...
            onClick={handleDeactivateRio}
          >
            {deactivating ? 'Desactivando…' : 'Desactivar RIO'}
          </button>
        )}
      </div>

      {rio?.descripcion && <p className="page-subtitle">{rio.descripcion}</p>}
      ...
      {!loading && tree && (
        <>
          ...
          {racksForRio.length === 0 && <p>Este RIO todavía no tiene racks activos.</p>}

          <div className="physical-tree">
            {racksForRio.map((rack) => (
              <RackBlock
                key={rack.id}
                projectId={projectId}
